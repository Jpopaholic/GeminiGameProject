...
每個遊戲都是一個獨立的資料夾，引擎掃描時會驗證以下兩個項目是否存在，缺一不可：

```
game_tools/
└── your_game/              ← 資料夾名稱即為 switch 指令的參數
    ├── plugin_config.json  ← 插件核心設定（必要，缺少則插件無效）
...
> [!IMPORTANT]
> `plugin_config.json` 與 `skills/` 資料夾**兩者皆必須存在**，插件才會被引擎識別為合法。若結構不完整，`switch` 切換時會收到警告訊息並列出缺少的項目，`skills/` 資料夾內容物可以是空的。
